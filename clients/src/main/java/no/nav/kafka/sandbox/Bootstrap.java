...
import no.nav.kafka.sandbox.experiment.ExperimentConsumer;
import no.nav.kafka.sandbox.experiment.ExperimentProducer;
...
public class Bootstrap {
    ...
    public static void main(String... a) {
        ...
        if (args.isEmpty() || args.get(0).isBlank() || args.contains("-h") || args.contains("--help")) {
            ...
            System.err.println("Use: 'experiment-consumer ID STRATEGY TIMEOUT HEARTBEAT GROUP [TOPIC]'");
            System.err.println("Use: 'experiment-producer RATE DURATION [TOPIC] [--hotspot]'");
            System.err.println("Example: ./run experiment-consumer 1 range 10000 3000 my-group experiment-topic");
            System.err.println("Example: ./run experiment-producer 50000 600 experiment-topic");
            System.err.println("Example: ./run experiment-producer 50000 600 experiment-topic --hotspot");
            System.exit(1);
        }
        ...
        try {
            switch (args.remove()) {
                ...
                case "experiment-consumer":
                    experimentConsumer(args);
                    break;

                case "experiment-producer":
                    experimentProducer(args);
                    break;
    ...
    private static void experimentConsumer(Queue<String> args) {
        String consumerId = args.remove();
        String strategy = args.remove();
        int sessionTimeout = Integer.parseInt(args.remove());
        int heartbeatInterval = Integer.parseInt(args.remove());
        String groupId = args.remove();
        String topic = args.isEmpty() ? "experiment-topic" : args.remove();

        LOG.info("Starting experiment consumer {}: strategy={}, timeout={}, heartbeat={}, group={}, topic={}",
                consumerId, strategy, sessionTimeout, heartbeatInterval, groupId, topic);

        // Создаем и запускаем consumer
        ExperimentConsumer consumer = new ExperimentConsumer(consumerId, strategy,
                sessionTimeout, heartbeatInterval, groupId, topic);
        consumer.start();

        // Ждём завершения потока
        Thread main = Thread.currentThread();
        Runtime.getRuntime().addShutdownHook(new Thread(() -> {
            LOG.info("Shutdown signal received for consumer {}", consumerId);
            consumer.stop();
            main.interrupt();
            try {
                main.join(3000);
        ...
        // Ждём завершения (теперь consumer управляет своим потоком)
        while (true) {
            try {
                Thread.sleep(1000);
            } catch (InterruptedException e) {
                LOG.info("Main thread interrupted, stopping consumer");
                consumer.stop();
                break;
            }
        }
    ...
    private static void experimentProducer(Queue<String> args) {
        long rate = Long.parseLong(args.remove());
        int duration = Integer.parseInt(args.remove());
        String topic = args.isEmpty() ? "experiment-topic" : args.remove();
        boolean hotspot = args.contains("--hotspot");

        var producer = new ExperimentProducer(rate, duration, topic, hotspot);
        producer.run();
    }

    private static void runFullExperiment(Queue<String> args) {
        LOG.info("Full experiment mode - нужно реализовать логику");
    }
